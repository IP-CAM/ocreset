...
if(isset($_POST['ID'])) {
    ...
    if($pass == '') {
        ...
        $query = sprintf("UPDATE ".DB_PREFIX."user SET salt =  '%s' , password = '%s' WHERE user_id = '%s'",   mysql_real_escape_string( $salt), mysql_real_escape_string($pass) , mysql_real_escape_string($_POST['ID']));
        $result = mysql_query($query);
        if(!$result) {
